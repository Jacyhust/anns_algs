...
void divGraph::insertLSHRefine(int pId)
{
	...
	if (pId != first_id && candTable.empty()) {
		candTable.emplace(first_id, cal_L2sqr(myData[pId], myData[first_id], dim));
...
void divGraph::knn(queryN* q)
{
		...
		if (f) {
			...
			for (int i = 0; i < step; ++i) {
				...
				if (flag_[res_pair.id] == 'U') {
					res_pair.dist = cal_L2sqr(q->queryPoint, myData[res_pair.id], dim);
	...
	timer.restart();
	if (candTable.empty()) {
		candTable.emplace(0, cal_L2sqr(q->queryPoint, myData[0], dim));
	}
